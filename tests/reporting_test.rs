...
async fn test_audit_trail_recording() {
    ...
    for i in 0..5 {
        let audit_event = AuditTrail {
            ...
            outcome: if i % 2 == 0 { AuditOutcome::Success } else { AuditOutcome::Failure },
    ...
    assert!(auth_trails.iter().all(|t| t.event_type == AuditEventType::Authentication));
}
...
async fn test_html_compliance_report_generation() {
    let compliance_report = ComplianceReport {
        ...
        findings: vec![],
        recommendations: vec![],
        executive_summary: "This is a test compliance report showing excellent results.".to_string(),
        next_assessment_date: Utc::now() + Duration::days(90),
    };
    
    let html_content = ReportingEngine::generate_html_compliance_report(&compliance_report).unwrap();
...
async fn test_text_compliance_report_generation() {
    let compliance_report = ComplianceReport {
        ...
        requirements: vec![
            ComplianceRequirement {
                ...
                implementation_status: ComplianceStatus::FullyCompliant,
                evidence_required: vec!["Policy documents".to_string(), "Training records".to_string()],
    ...
    let text_content = ReportingEngine::generate_text_compliance_report(&compliance_report).unwrap();
...
async fn test_default_reporting_config() {
    ...
    assert!(config.compliance_frameworks.contains(&ComplianceFramework::SOX));
    assert!(config.compliance_frameworks.contains(&ComplianceFramework::SOC2));
    assert!(config.compliance_frameworks.contains(&ComplianceFramework::ISO27001));
}