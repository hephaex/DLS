...
async fn test_create_raw_image() {
    ...
    let storage_manager = ZfsStorageManager::new(
        "test-pool".to_string(),
        "/tmp/test-images".to_string()
    );
    
    let image = storage_manager.create_image(
        "test-ubuntu", 
        1024 * 1024 * 1024, // 1GB
        ImageFormat::Raw
    ).await.unwrap();
...
async fn test_list_images() {
    ...
    let image1 = storage_manager.create_image(
        "ubuntu-20.04", 
        2 * 1024 * 1024 * 1024, // 2GB
        ImageFormat::Raw
    ).await.unwrap();
    
    let image2 = storage_manager.create_image(
        "windows-11", 
...
async fn test_resize_image() {
    ...
    storage_manager.resize_image(image.id, new_size).await.unwrap();
...
async fn test_create_snapshot() {
    ...
    let snapshot_name = storage_manager.create_snapshot(image.id, "test-snapshot").await.unwrap();
...
async fn test_clone_image() {
    ...
    let original = storage_manager.create_image(
        "original-image", 
        1024 * 1024 * 1024, // 1GB
        ImageFormat::Raw
    ).await.unwrap();
    
    let cloned = storage_manager.clone_image(original.id, "cloned-image").await.unwrap();
...
async fn test_image_formats() {
    ...
    let raw_image = storage_manager.create_image(
        "raw-image", 
        512 * 1024 * 1024,
        ImageFormat::Raw
    ).await.unwrap();
    ...
    if std::process::Command::new("qemu-img").arg("--version").output().is_ok() {
        let qcow2_image = storage_manager.create_image(
            "qcow2-image", 
            512 * 1024 * 1024,
            ImageFormat::Qcow2
        ).await.unwrap();
