...
async fn test_system_performance_metrics() {
    ...
    monitoring_manager.update_memory_available(2147483648.0).await; // 2GB in bytes
    monitoring_manager.update_network_throughput(1073741824.0).await; // 1GB/sec
...
async fn test_storage_metrics() {
    ...
    monitoring_manager.update_storage_used(1099511627776.0).await; // 1TB in bytes
    monitoring_manager.update_storage_available(2199023255552.0).await; // 2TB in bytes
...
async fn test_metrics_export() {
    ...
    assert!(exported_metrics.contains("TYPE") && 
...
async fn test_client_session_management() {
    ...
    monitoring_manager.start_client_session(client_id.clone(), ip_address.clone()).await.unwrap();
    ...
    monitoring_manager.update_client_status(&client_id, ClientStatus::ImageLoading).await.unwrap();
    monitoring_manager.update_client_status(&client_id, ClientStatus::Ready).await.unwrap();
    
    // End client session
    monitoring_manager.end_client_session(&client_id).await.unwrap();
