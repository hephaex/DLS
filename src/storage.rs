...
pub use zfs::{CompressionType, FreeBsdZfsManager, ZfsDataset, ZfsManager, ZfsSnapshot};
...
impl ZfsStorageManager {
    pub fn new(pool_name: String, base_path: String) -> Self {
        ...
        #[cfg(not(target_os = "freebsd"))]
        let zfs_manager: Box<dyn ZfsManager> =
            Box::new(zfs::MockZfsManager::new(pool_name.clone()));
    ...
    async fn load_image_metadata(&self) -> Result<()> {
        ...
        for dataset in datasets {
            if let Some(image_name) = dataset
                .name
                .strip_prefix(&format!("{}/images/", self.pool_name))
            {
    ...
    pub async fn create_image_dataset(&self, image_id: Uuid) -> Result<ZfsDataset> {
        #[cfg(target_os = "freebsd")]
        {
            ...
            properties.insert(
                "mountpoint".to_string(),
                format!("{}/{}", self.base_path, image_id),
            );
            properties.insert("compression".to_string(), "lz4".to_string());
    ...
    pub async fn create_image_snapshot(
        &self,
        image_id: Uuid,
        snapshot_name: &str,
    ) -> Result<ZfsSnapshot> {
...
impl StorageManager for ZfsStorageManager {
    async fn create_image(
        ...
        self.create_image_file(&image_path, size_bytes, &format)
            .await?;
    ...
    async fn delete_image(&self, id: Uuid) -> Result<()> {
        ...
        if let Ok(Some(_)) = self.zfs_manager.get_dataset(&dataset_name).await {
            self.zfs_manager
                .destroy_dataset(&dataset_name, true)
                .await?;
        }
    ...
    async fn clone_image(&self, id: Uuid, new_name: &str) -> Result<DiskImage> {
        ...
        self.zfs_manager
            .create_snapshot(&source_dataset, &snapshot_name)
            .await?;
        ...
        let _clone_dataset = self
            .zfs_manager
            .clone_snapshot(&snapshot_full_name, &clone_dataset)
            .await?;
    ...
    async fn restore_snapshot(&self, id: Uuid, snapshot_name: &str) -> Result<()> {
        ...
        let dataset_name = self.dataset_name_for_image(id);
        self.zfs_manager
            .rollback_snapshot(&dataset_name, snapshot_name)
            .await?;
        ...
        info!(
            "Successfully restored snapshot '{}' for image {}",
            snapshot_name, id
        );
        Ok(())
    }
