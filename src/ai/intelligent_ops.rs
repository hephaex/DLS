...
use crate::ai::predictive_analytics::{Prediction, PredictiveAnalyticsEngine};
...
impl IntelligentOpsEngine {
    ...
    pub async fn process_anomaly(&self, anomaly: &AnomalyRecord) -> Result<Vec<Uuid>> {
        ...
        for rule_entry in self.automation_rules.iter() {
            ...
            if self
                .evaluate_trigger_conditions(&rule.trigger_conditions, anomaly)
                .await?
            {
                ...
                let execution_id = self
                    .queue_execution(
                        rule.rule_id.clone(),
                        TriggerSource::Anomaly(anomaly.anomaly_id),
                    )
                    .await?;
    ...
    pub async fn process_prediction(&self, prediction: &Prediction) -> Result<Vec<Uuid>> {
        ...
        for rule_entry in self.automation_rules.iter() {
            ...
            if self
                .evaluate_prediction_triggers(&rule.trigger_conditions, prediction)
                .await?
            {
                let execution_id = self
                    .queue_execution(
                        rule.rule_id.clone(),
                        TriggerSource::Prediction(prediction.prediction_id),
                    )
                    .await?;
                triggered_executions.push(execution_id);
            }
            ...
            match condition.condition_type {
                ConditionType::AnomalyDetected => {
                    if condition.metric == anomaly.metric_name
                        && anomaly.anomaly_score >= condition.threshold
                    {
                        return Ok(true);
                    }
                }
                    ...
                    if condition.metric == anomaly.metric_name {
                        let meets_threshold = match condition.operator {
                            ComparisonOperator::GreaterThan => {
                                anomaly.observed_value > condition.threshold
                            }
                        ...
                        if meets_threshold
                            && anomaly.anomaly_score >= condition.confidence_threshold
                        {
                            return Ok(true);
                        }
                    }
    ...
    async fn evaluate_prediction_triggers(
        &self,
        conditions: &[TriggerCondition],
    ...
    async fn generate_performance_recommendations(&self) -> Result<Vec<IntelligentRecommendation>> {
        ...
        let recommendation = IntelligentRecommendation {
            ...
            description: "Increase database connection pool size to improve query performance"
                .to_string(),
            rationale: "Analysis shows frequent connection pool exhaustion during peak hours"
                .to_string(),
            confidence: 0.85,
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let next_execution = {
                    ...
                    queue
                        .iter()
                        .position(|e| !e.approval_required || e.approved_by.is_some())
    ...
    pub async fn approve_execution(&self, execution_id: Uuid, approved_by: String) -> Result<()> {
        ...
        if let Some(execution) = queue.iter_mut().find(|e| e.execution_id == execution_id) {
            ...
            Err(DlsError::Internal(
                "Execution not found in queue".to_string(),
            ))
        }
    }
