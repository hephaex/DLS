...
impl Metrics {
    pub fn new() -> Result<Self> {
        ...
        let dhcp_errors = Counter::new("dhcp_errors_total", "Total DHCP errors")
            .map_err(|e| DlsError::Internal(format!("Failed to create DHCP errors counter: {}", e)))?;
        
        let tftp_errors = Counter::new("tftp_errors_total", "Total TFTP errors")
            .map_err(|e| DlsError::Internal(format!("Failed to create TFTP errors counter: {}", e)))?;
        
        let iscsi_errors = Counter::new("iscsi_errors_total", "Total iSCSI errors")
            .map_err(|e| DlsError::Internal(format!("Failed to create iSCSI errors counter: {}", e)))?;
        
        let boot_sessions_total = Counter::new("boot_sessions_total", "Total boot sessions initiated")
            .map_err(|e| DlsError::Internal(format!("Failed to create boot sessions counter: {}", e)))?;
        
        let boot_failures = Counter::new("boot_failures_total", "Total boot failures")
            .map_err(|e| DlsError::Internal(format!("Failed to create boot failures counter: {}", e)))?;
        
        let storage_available_bytes = Gauge::new("storage_available_bytes", "Storage space available in bytes")
            .map_err(|e| DlsError::Internal(format!("Failed to create storage available gauge: {}", e)))?;
        
        let zfs_snapshots_total = Gauge::new("zfs_snapshots_total", "Total number of ZFS snapshots")
            .map_err(|e| DlsError::Internal(format!("Failed to create snapshots gauge: {}", e)))?;
        
        let image_operations = Counter::new("image_operations_total", "Total image operations")
            .map_err(|e| DlsError::Internal(format!("Failed to create image operations counter: {}", e)))?;
        
        let auth_requests = Counter::new("auth_requests_total", "Total authentication requests")
            .map_err(|e| DlsError::Internal(format!("Failed to create auth requests counter: {}", e)))?;
        
        let auth_failures = Counter::new("auth_failures_total", "Total authentication failures")
            .map_err(|e| DlsError::Internal(format!("Failed to create auth failures counter: {}", e)))?;
        
        let active_sessions = Gauge::new("active_sessions", "Number of active user sessions")
            .map_err(|e| DlsError::Internal(format!("Failed to create active sessions gauge: {}", e)))?;
        
        let token_refreshes = Counter::new("token_refreshes_total", "Total token refreshes")
            .map_err(|e| DlsError::Internal(format!("Failed to create token refreshes counter: {}", e)))?;
        ...
        let memory_available_bytes = Gauge::new("memory_available_bytes", "Available memory in bytes")
            .map_err(|e| DlsError::Internal(format!("Failed to create memory available gauge: {}", e)))?;
        
        let network_throughput_bytes = Gauge::new("network_throughput_bytes_per_sec", "Network throughput in bytes per second")
        ...
        let database_connections = Gauge::new("database_connections", "Number of active database connections")
            .map_err(|e| DlsError::Internal(format!("Failed to create database connections gauge: {}", e)))?;
        
        let database_queries = Counter::new("database_queries_total", "Total database queries")
            .map_err(|e| DlsError::Internal(format!("Failed to create database queries counter: {}", e)))?;
        
        let database_errors = Counter::new("database_errors_total", "Total database errors")
            .map_err(|e| DlsError::Internal(format!("Failed to create database errors counter: {}", e)))?;
        ...
        let health_checks = Counter::new("health_checks_total", "Total health checks performed")
            .map_err(|e| DlsError::Internal(format!("Failed to create health checks counter: {}", e)))?;
        
        let service_restarts = Counter::new("service_restarts_total", "Total service restarts")
            .map_err(|e| DlsError::Internal(format!("Failed to create service restarts counter: {}", e)))?;
    ...
    pub fn register_all(&self, registry: &Registry) -> Result<()> {
        ...
        registry.register(Box::new(self.dhcp_errors.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register DHCP errors: {}", e)))?;
        
        registry.register(Box::new(self.tftp_errors.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register TFTP errors: {}", e)))?;
        
        registry.register(Box::new(self.iscsi_errors.clone()))
        ...
        registry.register(Box::new(self.boot_sessions_total.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register boot sessions: {}", e)))?;
        
        registry.register(Box::new(self.boot_failures.clone()))
        ...
        registry.register(Box::new(self.storage_available_bytes.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register storage available: {}", e)))?;
        
        registry.register(Box::new(self.zfs_snapshots_total.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register ZFS snapshots: {}", e)))?;
        
        registry.register(Box::new(self.image_operations.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register image operations: {}", e)))?;
        
        registry.register(Box::new(self.auth_requests.clone()))
        ...
        registry.register(Box::new(self.active_sessions.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register active sessions: {}", e)))?;
        
        registry.register(Box::new(self.token_refreshes.clone()))
        ...
        registry.register(Box::new(self.network_throughput_bytes.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register network throughput: {}", e)))?;
        
        registry.register(Box::new(self.disk_io_bytes.clone()))
        ...
        registry.register(Box::new(self.database_connections.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register database connections: {}", e)))?;
        
        registry.register(Box::new(self.database_queries.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register database queries: {}", e)))?;
        
        registry.register(Box::new(self.database_errors.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register database errors: {}", e)))?;
        
        registry.register(Box::new(self.database_query_duration.clone()))
            .map_err(|e| DlsError::Internal(format!("Failed to register database query duration: {}", e)))?;
        ...
        registry.register(Box::new(self.service_restarts.clone()))
...
impl MonitoringManager {
    ...
    pub async fn record_database_query(&self, duration_seconds: f64) {
        ...
        self.metrics.database_query_duration.observe(duration_seconds);
    }
