...
impl TftpServer {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        info!(
            "Starting TFTP server with root: {:?}",
            self.config.root_path
        );
        info!("TFTP security mode: {:?}", self.config.security_mode);
        info!(
            "TFTP allowed extensions: {:?}",
        ...
        self.running = true;
        info!(
            "TFTP server started successfully on {}",
    ...
    async fn handle_tftp_request(
        ...
        match packet.opcode {
            ...
            TftpOpcode::Error => Self::handle_error(packet, client_addr, sessions).await,
            _ => {
                warn!(
                    "Unsupported TFTP opcode: {:?} from {}",
                    packet.opcode, client_addr
                );
                Ok(())
            }
        }
    ...
    async fn handle_read_request(
        ...
        // Check if too many sessions
        {
            ...
            if sessions_guard.len() >= config.max_sessions {
                ...
                socket
                    .send_to(&error_packet.to_bytes(), client_addr)
                    .await?;
                return Ok(());
            }
        }
        ...
        if parts.len() < 2 {
            let error_packet =
                TftpPacket::error(TftpErrorCode::IllegalOperation, "Invalid request format");
            socket
                .send_to(&error_packet.to_bytes(), client_addr)
                .await?;
            return Ok(());
        ...
        info!(
            "TFTP read request from {}: {} (mode: {})",
            client_addr, filename, mode_str
        );

        // Security check: validate filename
        if !Self::is_filename_allowed(&filename, &config) {
            warn!(
                "TFTP access denied for file '{}' from {}",
        ...
        if !file_path.starts_with(&config.root_path) {
            warn!(
                "Directory traversal attempt for '{}' from {}",
                filename, client_addr
            );
            let error_packet = TftpPacket::error(TftpErrorCode::AccessViolation, "Access denied");
            socket
        ...
        if !response_options.is_empty() {
            ...
            debug!(
                "Sent OACK to {} with options: {:?}",
                client_addr, response_options
            );
        } else {
    ...
    async fn handle_write_request(
        ...
        if !config.allow_write {
            ...
            let error_packet =
                TftpPacket::error(TftpErrorCode::AccessViolation, "Write not allowed");
            socket
                .send_to(&error_packet.to_bytes(), client_addr)
                .await?;
        } else {
            ...
            let error_packet =
                TftpPacket::error(TftpErrorCode::AccessViolation, "Write not implemented");
            socket
                .send_to(&error_packet.to_bytes(), client_addr)
                .await?;
        }
    ...
    async fn handle_ack(
        ...
        debug!(
            "Received ACK for block {} from {}",
            block_number, client_addr
        );
        ...
        if let Some(session) = sessions_guard.get_mut(&client_addr) {
            ...
            if block_number == session.current_block {
                ...
                if let Err(e) = Self::send_next_data_block(&file_path, session, socket).await {
    ...
    async fn handle_data(
        ...
        let error_packet =
            TftpPacket::error(TftpErrorCode::IllegalOperation, "Write not supported");
        socket
            .send_to(&error_packet.to_bytes(), client_addr)
            .await?;
        Ok(())
    ...
    async fn handle_error(
        ...
        if packet.data.len() >= 2 {
            ...
            warn!(
                "TFTP error from {}: code={}, message='{}'",
                client_addr, error_code, message
            );
        }
    ...
    async fn send_next_data_block(
        ...
        socket
            .send_to(&data_packet.to_bytes(), session.client_addr)
            .await?;
        ...
        if bytes_read < session.block_size {
            info!(
                "TFTP transfer completed: {} ({} bytes) to {}",
