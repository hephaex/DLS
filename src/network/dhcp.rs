...
impl DhcpServer {
    ...
    pub async fn add_reservation(&self, mac_address: [u8; 6], ip: Ipv4Addr) -> Result<()> {
        ...
        info!("Added DHCP reservation: {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} -> {}",
            mac_address[0], mac_address[1], mac_address[2], 
            mac_address[3], mac_address[4], mac_address[5], ip);
        Ok(())
    }
    ...
    pub async fn remove_reservation(&self, mac_address: [u8; 6]) -> Result<()> {
        ...
        if reservations.remove(&mac_address).is_some() {
            info!("Removed DHCP reservation for {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                mac_address[0], mac_address[1], mac_address[2], 
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        info!("DHCP options: server={}, subnet={}, gateway={:?}, DNS={:?}",
            self.options.server_ip, self.options.subnet_mask, 
            self.options.gateway, self.options.dns_servers);
        
        if let Some(tftp_server) = self.options.tftp_server {
            info!("PXE boot enabled: TFTP server={}, boot file={:?}", 
    ...
    async fn handle_dhcp_requests(socket: Arc<tokio::net::UdpSocket>, server: Arc<RwLock<DhcpServer>>) {
        ...
        loop {
            match socket.recv_from(&mut buf).await {
                Ok((len, addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::process_dhcp_packet(&data, addr, &socket_clone, server_clone).await {
    ...
    async fn process_dhcp_packet(
        ...
        debug!("DHCP request from MAC: {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
        ...
        match message_type {
            1 => { // DHCP DISCOVER
                info!("Processing DHCP DISCOVER from {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                    client_mac[0], client_mac[1], client_mac[2], 
                    client_mac[3], client_mac[4], client_mac[5]);
                Self::handle_dhcp_discover(&dhcp_packet, client_mac, socket, server).await?
            },
            3 => { // DHCP REQUEST
                info!("Processing DHCP REQUEST from {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                    client_mac[0], client_mac[1], client_mac[2], 
                    client_mac[3], client_mac[4], client_mac[5]);
                Self::handle_dhcp_request(&dhcp_packet, client_mac, socket, server).await?
            },
            7 => { // DHCP RELEASE
                info!("Processing DHCP RELEASE from {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                    client_mac[0], client_mac[1], client_mac[2], 
                    client_mac[3], client_mac[4], client_mac[5]);
    ...
    async fn handle_dhcp_discover(
        ...
        let offered_ip = if let Some(reserved_ip) = server_guard.reservations.read().await.get(&client_mac) {
            *reserved_ip
        ...
        socket.send_to(&offer, broadcast_addr).await
            .map_err(|e| DlsError::Network(format!("Failed to send DHCP OFFER: {}", e)))?;
        
        info!("Sent DHCP OFFER: {} to {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
            offered_ip,
            client_mac[0], client_mac[1], client_mac[2], 
            client_mac[3], client_mac[4], client_mac[5]);
    ...
    async fn handle_dhcp_request(
        ...
        let requested_ip = Self::get_requested_ip(&request.options)
            .unwrap_or_else(|| Ipv4Addr::new(request.ciaddr[0], request.ciaddr[1], request.ciaddr[2], request.ciaddr[3]));
        ...
        let broadcast_addr: SocketAddr = "255.255.255.255:68".parse().unwrap();
        socket.send_to(&ack, broadcast_addr).await
    ...
    async fn handle_dhcp_release(
        ...
        if let Some(lease) = leases.remove(&client_mac) {
            info!("Released DHCP lease: {} from {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                lease.ip,
                client_mac[0], client_mac[1], client_mac[2], 
                client_mac[3], client_mac[4], client_mac[5]);
        }
    ...
    async fn get_next_available_ip(&self) -> Result<Ipv4Addr> {
        ...
        for ip_u32 in start..=end {
            ...
            let is_leased = leases.values().any(|lease| 
        ...
        Err(DlsError::Network("No available IP addresses in range".to_string()))
    }
    ...
    fn create_dhcp_offer(request: &DhcpPacket, offered_ip: Ipv4Addr, options: &DhcpOptions) -> Result<Vec<u8>> {
        ...
        response[option_offset + 2..option_offset + 6].copy_from_slice(&options.subnet_mask.octets());
        option_offset += 6;
        ...
        if let Some(tftp_server) = options.tftp_server {
            ...
            response[option_offset + 2..option_offset + 2 + tftp_bytes.len()].copy_from_slice(tftp_bytes);
        ...
        if let Some(ref domain) = options.domain_name {
            ...
            response[option_offset + 2..option_offset + 2 + domain_bytes.len()].copy_from_slice(domain_bytes);
    ...
    fn create_dhcp_ack(request: &DhcpPacket, client_ip: Ipv4Addr, options: &DhcpOptions) -> Result<Vec<u8>> {
...
mod tests {
    ...
    fn test_dhcp_offer_creation() {
        ...
        while i < offer.len() - 2 {
            ...
            if offer[i] == 255 { break; }
            if offer[i] == 0 { i += 1; continue; }
            if i + 1 < offer.len() {
    ...
    fn test_hostname_parsing() {
        ...
        let mut options = vec![
            12, hostname.len() as u8, // Hostname option
        ];
